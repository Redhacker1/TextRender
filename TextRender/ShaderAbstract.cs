...
namespace TextRender
{
...

    {
        ...
        protected ShaderAbstract(ResourceFactory factory, string name, bool InvertY = false)
        {
            ...
            byte[] vertSpirvBytes = ReadEmbeddedAssetBytes($"TextRender.Shaders.{Name}.vert.spv");
            byte[] fragSpirvBytes = ReadEmbeddedAssetBytes($"TextRender.Shaders.{Name}.frag.spv");

            SpecializationConstant[] constants = null;

            if (factory.BackendType != GraphicsBackend.Vulkan || InvertY)
            {
                constants = new[] {new SpecializationConstant(0, false)};
            }
